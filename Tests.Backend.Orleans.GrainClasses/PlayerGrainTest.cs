...
public class PlayerGrainTest : TestKitBase {
    ...
    public async Task EnterChunk_ShouldMovePlayerToNewChunk() {
        ...
        currentChunkMock.Setup(x => x.GetNeighboringChunks(It.IsAny<int>()))
            .Returns(Task.FromResult<WorldChunkNeighbor[]>([]));
        ...
        targetChunkMock.Setup(x => x.AddPlayer(It.IsAny<string>(), playerName, It.IsAny<SerializableVector2>()))
            .Returns(Task.CompletedTask)
            .Verifiable(Times.Once);
        targetChunkMock.Setup(x => x.GetNeighboringChunks(It.IsAny<int>()))
            .Returns(Task.FromResult<WorldChunkNeighbor[]>([]));
    ...
    [Fact]
    public async Task StartMovementTimer_ShouldComplete() {
        // Arrange
        Guid playerGrainGuid = Guid.NewGuid();
        string playerName = "Test Player";

        Silo.AddServiceProbe<IRealtimeUpdatesOrleans>();
        ...
        // Act
        Task result = playerGrain.StartMovementTimer();
    ...
    [Fact]
    public async Task StartMovementTimer_ShouldRegisterTimerAndInvokeCallback() {
        // Arrange
        Guid playerGrainGuid = Guid.NewGuid();
        string playerName = "Test Player";
        string connectionId = "Connection123";
        ...
        realtimeUpdatesMock.Setup(x => x.AddToGroupAsync(chunkGroupName, connectionId))
            .Returns(Task.CompletedTask);
        realtimeUpdatesMock.Setup(x => x.PlayerMovementUpdate(
                chunkGroupName,
                It.IsAny<string>(),
                It.IsAny<int>(),
                It.IsAny<int>()))
        ...
        // Act
        await playerGrain.StartMovementTimer();
        ...
        SerializableVector2 newPosition = await playerGrain.GetPosition();
        Assert.NotEqual(oldPosition.X, newPosition.X);
        Assert.NotEqual(oldPosition.Y, newPosition.Y);
    }
