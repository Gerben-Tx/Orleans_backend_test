...
public interface IRealtimeUpdatesHub {
    Task<string> RegisterPlayerGrain(
        string playerName
    );

    Task<WorldChunkContract> GetCurrentChunk(
        string playerName
    );
    ...
    /// <returns></returns>
    Task DebugMoveToChunk(
        string playerName,
        int newChunkId
    );

    Task<List<PlayerListMessage>> GetPlayersInChunk(
        string playerName,
        long chunkId
    );

    Task<VisibleWorldChunksMessage> GetVisibleChunks(
        string playerName,
        int radius
    );
    ...
    Task<PlayerPositionMessage?> DebugGetPlayerPosition(
        string playerName
    );
}